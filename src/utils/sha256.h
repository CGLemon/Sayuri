...
inline void Transform(Context* ctx, const SHA256_BYTE* data) {
    size_t i, j;
...
inline void Update(Context* ctx, const SHA256_BYTE* data, size_t len) {
...
inline void Final(Context* ctx, SHA256_BYTE* hash) {
...
inline std::string GetDigest(const char* text, size_t len) {
    ...
    Initialize(&ctx);
    Update(&ctx, reinterpret_cast<const std::uint8_t*>(text), len);
    Final(&ctx, digest);
