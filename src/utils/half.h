...
single_float_t Rounded(single_float_t value, unsigned int g, unsigned int s) {
    // The 'g' is ground bit.
    // The 's' is sticky bit.
...
inline FpConvertStatus FloatToHalf(half_float_t& fp16, float fp32) {
    ...
    if (buf >= 0x38800000) {
        fp16 = Rounded<std::round_to_nearest>(sign | (((buf >> 23) - 112) << 10) |
                                                  ((buf >> 13) & 0x3FF),
                                              (buf >> 12) & 1,
                                              (buf & 0xFFF) != 0);
    ...
    if (buf >= 0x33000000) {
        ...
        fp16 =
            Rounded<std::round_to_nearest>(sign | (buf >> (i + 1)),
                                           (buf >> i) & 1,
