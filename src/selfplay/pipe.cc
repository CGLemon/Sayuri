...
void SelfPlayPipe::Initialize() {
    ...
    while (true) {
        ...
        ss << std::hex << std::uppercase << GetTimeHash() << std::dec;
...
bool SelfPlayPipe::SaveSgf(std::string& sgfstring) {
    ...
    auto sgf_filename = ConcatPath(sgf_directory_, filename_hash_ + ".sgf");
    bool is_open = SaveFile(sgf_filename, sgfstring);
    return is_open;
}

bool SelfPlayPipe::SaveChunk(const int out_id, float vdata_prob, std::vector<TrainingData>& chunk) {
    ...
    auto tdata_out_name =
        ConcatPath(tdata_directory_hash_, "game_" + std::to_string(out_id) + ".txt");
    auto vdata_out_name =
        ConcatPath(vdata_directory_hash_, "game_" + std::to_string(out_id) + ".txt");
...
bool SelfPlayPipe::SaveNetQueries(int games, std::string net_queries) {
    auto out_name = ConcatPath(queies_directory_, filename_hash_ + ".txt");
    ...
    bool is_open = file.is_open();
    if (is_open) {
        file << games << " " << net_queries << std::endl;
    } else {
        LOGGING << "Fail to create the file: " << out_name << '!' << std::endl;
    }
            ...
            while (accumulation_games_.fetch_add(1) < max_games_) {
...

                {
                    ...
                    if (played_games % kVerboseGames == 0) {
                        LOGGING << '[' << CurrentDateTime() << ']' << " Played " << played_games
                                << " games." << std::endl;
                    }
                    // The accumulated value throughout the process may be inaccurate,
                    // but the final value is correct.
                    games_queries_buffer_.emplace_back(played_games, engine_.GetNetReportQueries());
                }
            }
...
void SelfPlayPipe::Loop() {
    ...
    if (max_games_ < engine_.GetParallelGames()) {
        ...
        LOGGING << "WARNING: The number of self-play games must be greater than parallel games. "
                   "New value is "
                << max_games_ << "." << std::endl;
    }
    if (!IsGzipValid()) {
        LOGGING << "WARNING: There is no gzip tool. The output chunks may be very large."
                << std::endl;
    }
    CreateWorkspace();
    ...
    LOGGING << '[' << CurrentDateTime() << ']' << " Finish the self-play loop. Totally played "
