...
constexpr std::uint16_t ComputePattern3HashFromList(int* buf) {
    const std::uint16_t hash = buf[0] << 0 | buf[1] << 2 | buf[2] << 4 | buf[3] << 6 /* raw[4] */ |
                               buf[5] << 8 | buf[6] << 10 | buf[7] << 12 | buf[8] << 14;
    return hash;
}
        ...
        while (!pat3_raw_que.empty()) {
            ...
            for (int y = 0; y < 3; ++y) {
                for (int x = 0; x < 3; ++x) {
                    ...
                    switch (m) {
                        case 'x':
                            extension = Pattern3Replacement(pat3_raw, y, x, {'O', '.'});
                            break;
                        case 'o':
                            extension = Pattern3Replacement(pat3_raw, y, x, {'X', '.'});
                            break;
                        case '?':
                            extension = Pattern3Replacement(pat3_raw, y, x, {'X', 'O', '#', '.'});
                            break;
                        default:;
                    }
                    if (!extension.empty()) {
    ...
    for (auto& pat3 : symm_pat3_buf) {
        ...
        for (int y = 0; y < 3; ++y) {
            for (int x = 0; x < 3; ++x) {
                ...
                switch (m) {
                    case 'X':
                        list.emplace_back(kBlack);
                        break;
                    case 'O':
                        list.emplace_back(kWhite);
                        break;
                    case '.':
                        list.emplace_back(kEmpty);
                        break;
                    case '#':
                        list.emplace_back(kInvalid);
                        break;
                    default:
                        throw "unknown Patterns...";
                }
            }
        }
...
bool Board::MatchPattern3(const int vtx) const {
    ...
    constexpr int kColorMap[2][4] = {{kBlack, kWhite, kEmpty, kInvalid},
                ...
                for (int x = 0; x < 3; ++x) {
                    ...
                    switch (m) {
                        ...
                        case 'O':
                            ...
                            break;
                        case 'x':
                            if (s != kWhite && s != kEmpty) {
                                match = false;
                            }
                            break;
                        case 'o':
                            if (s != kBlack && s != kEmpty) {
                                match = false;
                            }
                            break;
                        case '.':
...
std::uint16_t
Board::GetSymmetryPattern3Hash(const int vtx, const int color, const int symmetry) const {
    int size = letter_box_size_;
    int buf[9];
    ...
    for (int y = 0; y < 3; ++y) {
        for (int x = 0; x < 3; ++x) {
            ...
            if (symmetry & PTH_180ROT)
                std::swap(rx, ry);
            if (symmetry & PTH_HMIRROR)
                rx = 2 - rx;
            if (symmetry & PTH_VMIRROR)
                ry = 2 - ry;
...
std::string Board::GetPatternSpat(const int vtx, const int color, const int dist) const {
    ...
    for (int i = kPointIndex[2]; i < kPointIndex[dist + 1]; ++i) {
        ...
        if (px >= board_size_ || py >= board_size_ || px < 0 || py < 0) {
            out << '#'; // invalid
        } else {
...
std::uint64_t Board::GetSymmetryPatternHash(const int vtx,
                                            const int color,
                                            const int dist,
...
bool Board::GetCapureLevel(const int vtx, const int color, std::uint64_t& hash) const {
    ...
    for (int k = 0; k < 4; ++k) {
        ...
        if (state_[avtx] == opp_color && GetLiberties(avtx) == 1) {
            ...
            if (std::end(pset) == std::find(std::begin(pset), std::end(pset), parent)) {
                ...
                if (IsLadder(avtx, vital_moves)) {
                    if (std::end(vital_moves) !=
                        std::find(std::begin(vital_moves), std::end(vital_moves), vtx)) {
...
bool Board::GetFeatureWrapper(const int f,
                              const int vtx,
                              const int color,
                              std::uint64_t& hash) const {
    switch (f) {
        case 0:
            return GetBorderLevel(vtx, color, hash);
        case 1:
            return GetDistLevel(vtx, color, hash);
        case 2:
            return GetDistLevel2(vtx, color, hash);
        case 3:
            return GetCapureLevel(vtx, color, hash);
        case 4:
            return GetAtariLevel(vtx, color, hash);
        case 5:
            return GetSelfAtariLevel(vtx, color, hash);
    }
    return false;
}
