...
static double NormToTApprox(double z, double n) {
    ...
    return std::sqrt(n * exp(z * z * (n-a) / ((n-b) * (n-c))) - n);
}
...
private:
    ...
    std::array<float, kEntrySize> _z_lookup_table;
};
...
inline void LcbEntries::Initialize(float complement_probability) {
    ...
    for (int i = 0; i < kEntrySize; ++i) {
        _z_lookup_table[i] = NormToTApprox(z, i+2);
    }
}

inline float LcbEntries::CachedTQuantile(int v) {
    if (v < 1) {
        return _z_lookup_table[0];
    }
    if (v < kEntrySize) {
        return _z_lookup_table[v - 1];
    }
    ...
    return _z_lookup_table[kEntrySize - 1];
}
