...
template <typename NodeType> class NodePointer {
public:
    NodePointer() = default;
...
template <typename NodeType> inline NodePointer<NodeType>::NodePointer(NodePointer&& n) {
...
template <typename NodeType> inline bool NodePointer<NodeType>::IsPointer(std::uint64_t v) const {
    return (v & POINTER_MASK) == kPointer;
}

template <typename NodeType> inline bool NodePointer<NodeType>::IsInflating(std::uint64_t v) const {
...
template <typename NodeType> inline bool NodePointer<NodeType>::IsPointer() const {
    return IsPointer(pointer_.load(std::memory_order_relaxed));
}

template <typename NodeType> inline bool NodePointer<NodeType>::IsInflating() const {
    return IsInflating(pointer_.load(std::memory_order_relaxed));
}

template <typename NodeType> inline bool NodePointer<NodeType>::IsUninflated() const {
    return IsUninflated(pointer_.load(std::memory_order_relaxed));
}

template <typename NodeType> inline NodeType* NodePointer<NodeType>::ReadPointer(uint64_t v) const {
    assert(IsPointer(v));
    return reinterpret_cast<NodeType*>(v & ~(POINTER_MASK));
}

template <typename NodeType> inline NodeType* NodePointer<NodeType>::GetPointer() const {
...
template <typename NodeType> inline bool NodePointer<NodeType>::Inflate(Parameters* param) {
...
inflate_loop: // Try to allocate new memory for the pointer.
    ...
    auto new_pointer =
        reinterpret_cast<std::uint64_t>(new NodeType(param, vertex, policy)) | kPointer;
...
template <typename NodeType> inline bool NodePointer<NodeType>::Release() {
...
template <typename NodeType> inline int NodePointer<NodeType>::ReadVertex(std::uint64_t v) const {
    std::int16_t res;
...
template <typename NodeType> inline float NodePointer<NodeType>::ReadPolicy(std::uint64_t v) const {
    float res;
...
template <typename NodeType> inline int NodePointer<NodeType>::GetVertex() const {
    auto v = pointer_.load(std::memory_order_relaxed);
...
template <typename NodeType> inline float NodePointer<NodeType>::GetPolicy() const {
    auto v = pointer_.load(std::memory_order_relaxed);
...
template <typename NodeType> inline int NodePointer<NodeType>::GetVisits() const {
    auto v = pointer_.load(std::memory_order_relaxed);
