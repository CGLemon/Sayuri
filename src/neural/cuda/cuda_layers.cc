...
void AddVectors(bool fp16,
    ...
    if (fp16) {
#ifdef ENABLE_FP16
        add_vectors((half*)c, (const half*)a, (const half*)b, size, asize, bsize, act, stream);
#endif
    } else {
        add_vectors((float*)c, (const float*)a, (const float*)b, size, asize, bsize, act, stream);
    }
}

void AddSpatial(bool fp16,
                void* data,
                const void* biases,
                const void* residual,
                const void* mask,
...
#ifdef ENABLE_FP16
        im2col_batched(
            (half*)data_col, (half*)data_im, filter_size, batch, channels, height, width, stream);
#endif
    } else {
        im2col_batched(
            (float*)data_col, (float*)data_im, filter_size, batch, channels, height, width, stream);
    }
}

void NormalGlobalPooling(bool fp16,
                         void* output,
                         void* input,
                         const void* mask,
                         const void* sqrt_mask,
                         int batch,
                         int channels,
                         int spatial,
                         cudaStream_t stream) {
    if (fp16) {
#ifdef ENABLE_FP16
        global_pooling((half*)output,
                       (half*)input,
                       (const half*)mask,
                       (const half*)sqrt_mask,
                       batch,
                       channels,
                       spatial,
                       stream);
...
void HeadGlobalPooling(bool fp16,
                       void* output,
                       void* input,
                       const void* sqrt_mask,
                       int batch,
...
#ifdef ENABLE_FP16
        head_global_pooling(
            (half*)output, (half*)input, (const half*)sqrt_mask, batch, channels, spatial, stream);
#endif
    } else {
        head_global_pooling((float*)output,
                            (float*)input,
                            (const float*)sqrt_mask,
                            batch,
                            channels,
...
void SeScale(bool fp16,
             void* output,
             const void* input,
             const void* residual,
             const void* se_biases,
             const void* mask,
             int batch,
             int channels,
             int spatial,
    ...
    if (fp16) {
#ifdef ENABLE_FP16
        se_scale((half*)output,
                 (const half*)input,
                 (const half*)residual,
                 (const half*)se_biases,
                 (const half*)mask,
                 batch,
                 channels,
                 spatial,
        ...
        se_scale((float*)output,
                 (const float*)input,
                 (const float*)residual,
                 (const float*)se_biases,
                 (const float*)mask,
                 batch,
                 channels,
                 spatial,
...
void Winograd3TransformIn(bool fp16,
                          void* output,
                          const void* input,
                          int batch,
                          int channels,
    ...
    if (fp16) {
        ...
        winograd3_transform_in(
            (half*)output, (const half*)input, batch, channels, board_size, stream);
#endif
    } else {
        winograd3_transform_in(
            (float*)output, (const float*)input, batch, channels, board_size, stream);
    }
}

void Winograd3TransformOut(bool fp16,
                           void* output,
                           const void* input,
                           const void* biases,
                           const void* residual,
    ...
    if (fp16) {
#ifdef ENABLE_FP16
        winograd3_transform_out((half*)output,
                                (const half*)input,
                                (const half*)biases,
                                (const half*)residual,
                                (const half*)mask,
        ...
        winograd3_transform_out((float*)output,
                                (const float*)input,
                                (const float*)biases,
                                (const float*)residual,
                                (const float*)mask,
...
void GemmStridedBatched(bool fp16,
                        bool TA,
                        bool TB,
                        int M,
                        int N,
...
void Convolution::Forward(const int batch,
    ...
    ReportCUDNNErrors(cudnnSetTensor4dDescriptor(
        in_tensor_desc_, CUDNN_TENSOR_NCHW, cudnn_data_type, batch, in_channels_, height_, width_));
    ...
    ReportCUDNNErrors(cudnnConvolutionForward(handles_->cudnn_handle,
                                              &alpha,
                                              in_tensor_desc_,
                                              input,
                                              filter_desc_,
    ...
    if (winograd_) {
        ...
        Winograd3TransformIn(
            fp16_, scratch_op, input, batch, in_channels_, board_size, handles_->stream);
        GemmStridedBatched(fp16_,
                           true,
                           false,
                           out_channels_,
                           batch_ptiles,
                           ...
                           scratch_op,
                           batch_ptiles,
                           in_channels_ * batch_ptiles,
                           0.0f,
                           scratch_op_other,
                           batch_ptiles,
                           out_channels_ * batch_ptiles,
                           kWinogradTile,
                           handles_->cublas_handle);
        Winograd3TransformOut(fp16_,
                              output,
                              scratch_op_other,
                              cuda_biases_,
                              residual,
        ...
        if (filters_ != 1) {
            ...
            GemmStridedBatched(fp16_,
                               ...
                               scratch_op,
                               spatial_size_,
                               filter_dim_ * spatial_size_,
                               0.f,
                               output,
                               spatial_size_,
                               out_channels_ * spatial_size_,
                               batch,
                               handles_->cublas_handle);
        } else {
            GemmStridedBatched(fp16_,
...
void Convolution::LoadWeights(const std::vector<float>& weights,
    ...
    if (winograd_) {
        weights_copy = WinogradTransformF(weights_copy, out_channels_, in_channels_);
    }
    ...
    ReportCUDNNErrors(cudnnSetFilter4dDescriptor(filter_desc_,
                                                 cudnn_data_type,
                                                 CUDNN_TENSOR_NCHW,
                                                 out_channels_,
                                                 in_channels_,
    ...
    ReportCUDNNErrors(cudnnSetConvolution2dDescriptor(
        conv_desc_, padding, padding, 1, 1, 1, 1, CUDNN_CROSS_CORRELATION, cudnn_data_type));
    ...
    if (winograd_) {
        scratch_size_base =
            kWinogradTile * GetWinogradP(board_size) * std::max(out_channels_, in_channels_);
    } else {
...
void Convolution::LoadWeights(const std::vector<float>& weights,
    ...
    cudnnDataType_t cudnn_data_type = GetCudnnDataType(fp16_);
    ReportCUDNNErrors(cudnnSetTensor4dDescriptor(
        bias_desc_, CUDNN_TENSOR_NCHW, cudnn_data_type, 1, out_channels_, 1, 1));
#endif
...
void DepthwiseConvolution::Forward(
    const int batch, void* output, void* input, const void* residual, const void* mask) {
    if (!loaded_) {
        return;
    }
    assert(batch <= maxbatch_);
...
void GlobalPooling::Forward(
    const int batch, void* output, void* input, void* mask, void* sqrt_mask) {
    if (is_value_head_) {
        HeadGlobalPooling(
            fp16_, output, input, sqrt_mask, batch, channels_, spatial_size_, handles_->stream);
    } else {
        NormalGlobalPooling(fp16_,
                            output,
                            input,
                            mask,
                            sqrt_mask,
...
void SEUnit::Forward(
    const int batch, void* ouput, void* input, void* residual, void* mask, void* sqrt_mask) {
    if (!loaded_) {
        return;
    }
    NormalGlobalPooling(fp16_,
