...
void Blas::ConvolutionSgemm(const int M,
                            const int N,
                            const int K,
                            const float alpha,
                            const float* A,
                            ...
                            const int ldc) {
#ifndef USE_BLAS
    Sgemm<false, false>::apply(M, N, K, alpha, A, lda, B, ldb, beta, C, ldc);
#else
#ifdef USE_OPENBLAS
    cblas_sgemm(
    ...
    C_mat.noalias() = ConstEigenMatrixMap<float>(B, N, K) * ConstEigenMatrixMap<float>(A, K, M);
#endif
#endif
}

void Blas::WinogradSgemm(const int offset_u,
                         const int offset_v,
                         const int offset_m,
                         const int M,
                         const int N,
...
#ifndef USE_BLAS
    Sgemm<true, false>::apply(
        M, N, K, alpha, A + offset_u, lda, B + offset_v, ldb, beta, C + offset_m, ldc);
    ...
    C_mat.noalias() = ConstEigenMatrixMap<float>(B + offset_v, N, K) *
...
void Blas::DenseSgemm(const int input_size,
...
#ifndef USE_BLAS
    Sgemm<false, true>::apply(batch_size,
                              output_size,
                              input_size,
                              kAlpha,
                              inputs,
    ...
    if (batch_size == 1) {
        ...
        y.noalias() = ConstEigenMatrixMap<float>(kernel, input_size, output_size).transpose() *
                      ConstEigenVectorMap<float>(inputs, input_size);
    } else {
        auto C_mat = EigenMatrixMap<float>(outputs, output_size, batch_size);
        C_mat.noalias() = ConstEigenMatrixMap<float>(kernel, input_size, output_size).transpose() *
