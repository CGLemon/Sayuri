...
OutputResult BlasForwardPipe::Forward(const InputData &inpnts) {
    ...
    const auto intersections = board_size * board_size;
    ...
    const auto plane_size = kInputChannels * intersections;


    // Allocate the forward pipe buffers. 
    auto workspace = std::vector<float>( Convolution3::GetWorkspaceSize(board_size, max_channels));
    auto conv_out = std::vector<float>(output_channels * kInputChannels);
    auto conv_in = std::vector<float>(output_channels * kInputChannels);
    auto res = std::vector<float>(output_channels * kInputChannels);
    ...
    auto output_prob = std::vector<float>(intersections);
    auto output_pass = std::vector<float>(kOuputPassProbability);
    auto output_ownership = std::vector<float>(intersections);
    ...
    auto policy_conv = std::vector<float>(policy_extract_channels * intersections);
    ...
    auto value_conv = std::vector<float>(value_extract_channels * intersections);
    ...
    result.winrate = output_misc[0];
    result.final_score = output_misc[1];
    result.score_width = output_misc[2];
