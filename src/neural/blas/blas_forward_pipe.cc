...
void BlasForwardPipe::Convolution3Forward(const int board_size,
    ...
    if (weights_->winograd) {
        WinogradConvolution3::Forward(board_size,
                                      input_channels,
                                      output_channels,
                                      input,
                                      conv_layer.GetTransformF(),
                                      workspace0,
                                      workspace1,
                                      output);
    } else {
        Convolution<3>::Forward(board_size,
                                input_channels,
                                output_channels,
                                input,
                                conv_layer.GetWeights(),
                                workspace0,
                                output);
    }
}
...
void BlasForwardPipe::ResidualBlockForward(const int board_size,
    ...
    // 1st conv3x3
    Convolution3Forward(board_size,
                        channels,
                        channels,
                        tower_ptr->conv1,
                        conv_in,
    ...
    AddSpatialBiases::Forward(
        board_size, channels, conv_out, tower_ptr->conv1.GetBiases(), default_act);
    ...
    AddSpatialBiases::Forward(
        board_size, channels, conv_out, tower_ptr->conv2.GetBiases(), last_skip, last_act);
}
...
void BlasForwardPipe::BottleneckBlockForward(const int board_size,
    ...
    Convolution1::Forward(board_size,
                          outer_channels,
                          inner_channels,
                          conv_in,
                          ...
                          conv_out);
    AddSpatialBiases::Forward(
        board_size, inner_channels, conv_out, tower_ptr->pre_btl_conv.GetBiases(), default_act);
    ...
    Convolution3Forward(board_size,
                        ...
                        conv_out);
    AddSpatialBiases::Forward(
        board_size, inner_channels, conv_out, tower_ptr->conv1.GetBiases(), default_act);
    ...
    Convolution3Forward(board_size,
                        ...
                        conv_out);
    AddSpatialBiases::Forward(
        board_size, inner_channels, conv_out, tower_ptr->conv2.GetBiases(), default_act);
    ...
    AddSpatialBiases::Forward(board_size,
                              outer_channels,
                              conv_out,
...
void BlasForwardPipe::NestedBottleneckBlockForward(const int board_size,
    ...
    Convolution3Forward(board_size,
                        ...
                        conv_out);
    AddSpatialBiases::Forward(
        board_size, inner_channels, conv_out, tower_ptr->conv2.GetBiases(), residual1, default_act);
    std::swap(conv_out, conv_in);
    ...
    Convolution3Forward(board_size,
                        ...
                        conv_out);
    AddSpatialBiases::Forward(
        board_size, inner_channels, conv_out, tower_ptr->conv3.GetBiases(), default_act);
    std::swap(conv_in, residual1);
    std::swap(conv_out, conv_in);
    ...
    Convolution3Forward(board_size,
                        ...
                        conv_out);
    AddSpatialBiases::Forward(
        board_size, inner_channels, conv_out, tower_ptr->conv4.GetBiases(), residual1, default_act);
    std::swap(conv_out, conv_in);
...
void BlasForwardPipe::MixerBlockForward(const int board_size,
    ...
    DepthwiseConvolution::Forward(
        board_size, filter_size, channels, conv_in, tower_ptr->dw_conv.GetWeights(), conv_out);
    AddSpatialBiasesPost::Forward(
        board_size, channels, conv_out, tower_ptr->dw_conv.GetBiases(), default_act, conv_in);
    ...
    Convolution1::Forward(board_size,
                          ...
                          conv_out);
    AddSpatialBiases::Forward(
        board_size, ffn_channels, conv_out, tower_ptr->conv1.GetBiases(), default_act);
...
OutputResult BlasForwardPipe::Forward(const InputData& inpnts) {
    ...
    for (int i = 0; i < weights_->residual_blocks; ++i) {
        ...
        if (tower_ptr->IsResidualBlock()) {
            ...
            tower_peak_channels = std::max(tower_peak_channels, tower_ptr->bottleneck_channels);
        } else if (tower_ptr->IsMixerBlock()) {
            tower_peak_channels = std::max(tower_peak_channels, tower_ptr->feedforward_channels);
        }
    }
    ...
    const auto max_intermediates =
        std::max(weights_->policy_head_channels, weights_->value_head_channels);
    const auto default_act = weights_->default_act;
    ...
    if (weights_->winograd) {
        workspace0_size = workspace1_size =
            WinogradConvolution3::GetWorkspaceSize(board_size, max_channels);
    } else {
        workspace0_size = Convolution<3>::GetWorkspaceSize(board_size, max_channels);
    ...
    std::copy(
        std::begin(inpnts.planes), std::begin(inpnts.planes) + plane_size, std::begin(planes));
    ...
    Convolution3Forward(board_size,
                        ...
                        conv_out);
    AddSpatialBiases::Forward(
        board_size, residual_channels, conv_out, weights_->input_conv.GetBiases(), default_act);
    ...
    for (int i = 0; i < weights_->residual_blocks; ++i) {
        ...
        if (tower_ptr->IsResidualBlock()) {
            ResidualBlockForward(
                board_size, tower_ptr, residual0, conv_in, conv_out, workspace0, workspace1);
        } else if (tower_ptr->IsBottleneckBlock()) {
            BottleneckBlockForward(
                board_size, tower_ptr, residual0, conv_in, conv_out, workspace0, workspace1);
        } else if (tower_ptr->IsNestedBottleneckBlock()) {
            NestedBottleneckBlockForward(board_size,
                                         tower_ptr,
                                         residual0,
                                         residual1,
                                         conv_in,
            ...
            MixerBlockForward(board_size, tower_ptr, residual0, conv_in, conv_out);
        }
    ...
    if (weights_->policy_head_type == PolicyHeadType::kRepLK) {
        ...
        DepthwiseConvolution::Forward(board_size,
                                      filter_size,
                                      policy_head_channels,
                                      policy_conv,
    ...
    GlobalPooling<false>::Forward(board_size, policy_head_channels, policy_conv, pooling);

    FullyConnect::Forward(3 * policy_head_channels,
                          policy_head_channels,
                          pooling,
    ...
    AddSpatialBiases::Forward(
        board_size, policy_head_channels, policy_conv, intermediate, Activation::kIdentity);

    // The policy outs.
    Convolution1::Forward(board_size,
                          policy_head_channels,
                          weights_->probabilities_channels,
                          policy_conv,
    ...
    FullyConnect::Forward(policy_head_channels,
                          weights_->pass_probability_outputs,
                          intermediate,
    ...
    AddSpatialBiases::Forward(
        board_size, value_head_channels, value_conv, weights_->v_hd_conv.GetBiases(), default_act);

    GlobalPooling<true>::Forward(board_size, value_head_channels, value_conv, pooling);

    FullyConnect::Forward(3 * value_head_channels,
                          3 * value_head_channels,
                          pooling,
    ...
    FillOutputs(output_prob, output_pass, output_misc, output_ownership, inpnts, result);
...
void BlasForwardPipe::FillOutputs(const std::vector<float>& output_prob,
    ...
    if (encoder_version == 1) {
        ...
        auto pol_it = std::begin(output_prob) + (int)inpnts.offset * num_intersections;
        std::copy(pol_it, pol_it + num_intersections, std::begin(output.probabilities));
